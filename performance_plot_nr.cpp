 ...
 *
 * Compare performance and accuracy of the ARPACK, Eigen and Numerical Recipes
 * eigensolvers.
 *
 * Computes execution time of the ARPACK, Eigen and the Numerical Recipes
 * eigensolver for random symmetric matrices for a range of matrix dimensions.
 * Computes the mean error of the ARPACK eigenvalues and eigenvectors as well.
 * The number of eigenvalues to be computed (for ARPACK) and the number of zeros
 * in each matrix can be specified as a percentage of the matrix dimension.
 * Produces output suitable for plotting.
 *
 ...
 *    -O3 \
 *    -DNDEBUG \
 *    performance_plot_nr.cpp \
 *    -L [/path/to/libarpack.a] \
 *    -larpack \
 *    -o performance_plot_nr
 *
 * This assumes that arpaca.hpp is in the same directory.
 *
 * We can't redistribute the Numerical Recipes headers. Put them in a subfolder
 * `nr` to compile this code.
 */
...
VectorXd diagonalize_random_matrix(int n_dim, double r_ev, double r_zeros)
{
    ...
    /*
     * Diagonalize with numerical recipes
     */
    steady_clock::time_point begin_n;
    steady_clock::time_point end_n;
    VecDoub eigenvalues_n_(n_dim);
    MatDoub eigenvectors_n_(n_dim,n_dim);
    Eigen::VectorXd eigenvalues_n(n_ev);
    Eigen::MatrixXd eigenvectors_n(n_ev,n_ev);
    MatDoub m_n(n_dim,n_dim);
    for (int j=0; j<n_dim; j++)
        for (int i=0; i<n_dim; i++)
            m_n[i][j] = sm.coeffRef(i,j); // slow
    begin_n = steady_clock::now();
    Symmeig s_n(m_n);
    end_n = steady_clock::now();
    eigenvalues_n_ = s_n.d;
    eigenvectors_n_ = s_n.z;
    for (int i=0; i<n_ev; i++)
        eigenvalues_n(i) = eigenvalues_n_[n_dim-i-1];
    for (int j=0; j<n_ev; j++)
        for (int i=0; i<n_ev; i++)
            eigenvectors_n(i,j) = eigenvectors_n_[i][n_dim-j-1];
    
    duration<double> d_n = duration_cast<duration<double>>(end_n - begin_n);
    double time_n = d_n.count();

    /*
     * Calculate error
     */
    double error_values_ae = (eigenvalues_a - eigenvalues_e).cwiseAbs().mean();
    double error_vectors_ae = 0;
    for (int i = 0; i < n_ev; i++)
    {
        double d1 = 
        ...
        double d2 =
            (eigenvectors_a.col(i) + eigenvectors_e.col(i)).cwiseAbs().mean();
        error_vectors_ae += std::min(d1, d2);
    }
    error_vectors_ae /= n_ev;

    // Note: eigenvectors of Numerical Recipes do not seem to agree, I am not
    // sure why.
    double error_values_ne = (eigenvalues_n - eigenvalues_e).cwiseAbs().mean();
    double error_vectors_ne = 0;
    for (int i = 0; i < n_ev; i++)
    {
        double d1 = 
            (eigenvectors_n.col(i) - eigenvectors_e.col(i)).cwiseAbs().mean();
        double d2 =
            (eigenvectors_n.col(i) + eigenvectors_e.col(i)).cwiseAbs().mean();
        error_vectors_ne += std::min(d1, d2);
    }
    error_vectors_ne /= n_ev;
    ...
    std::cout << "Numerical recipes eigenvalues: " << std::endl;
    std::cout << eigenvalues_n << std::endl << std::endl;
    std::cout << "Time in seconds of Arpaca: " << time_a << std::endl;
    std::cout << "Time in seconds of Eigen: " << time_e << std::endl;
    std::cout << "Time in seconds of Numerical Recipes: " << time_n << std::endl;
    std::cout << "Mean error of eigenvalues (Arpaca - Eigen): " << error_values_ae << std::endl;
    std::cout << "Mean error of eigenvectors (Arpaca - Eigen): " << error_vectors_ae << std::endl;
    std::cout << "Mean error of eigenvalues (NR - Eigen): " << error_values_ne << std::endl;
    std::cout << "Mean error of eigenvectors (NR - Eigen): " << error_vectors_ne << std::endl;
    ...
    */
     ...
     */
    VectorXd r(9);
    r << time_a, time_e, time_n, 
         error_values_ae, error_vectors_ae, 
         error_values_ne, error_vectors_ne, 
...
{
    std::cerr 
        << "Performance and accuracy of the ARPACK, Eigen and Numerical"
        << std::endl
        << "Recipes eigensolvers." << std::endl << std::endl
...
void print_header(int n_rep, double r_ev, double r_zeros, int w)
{
    ...
    std::cout 
        ...
        << std::setw(w) << "time_e"
        << std::setw(w) << "time_n"
        << std::setw(w) << "error_values_ae"
        << std::setw(w) << "error_vectors_ae"
        << std::setw(w) << "error_values_ne"
        << std::setw(w) << "error_vectors_ne"
        << std::setw(w) << "n_it"
        << std::setw(w) << "n_conv"
        << std::endl;
}
